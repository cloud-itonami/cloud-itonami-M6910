...
- **Store** -- `MemStore`（既定・依存なし）。`langchain.db` 経由の
  `DatomicStore` / kotoba-server backend への差し替えは次のシームとして
  用意されている（`cloud-itonami-6310` / `ai-gftd-itonami` と同じ
  `:db-api` 駆動パターンを想定、本 R0 では未実装）。
...
`formation.store/MemStore` は customer application / officer directory /
KYC 結果 / assessment 結果 / 台帳(ledger) / draft registry history を持つ。
commit するのは `:commit` ノードだけ。hold した提案も台帳には残る
（何を・なぜ止めたかが追跡できる）。
